...
export default function AssignmentItem({ asDetails, setShowModal, assignId }) {
  const [deleteAssignment, { isLoading, isError, isSuccess }] =
    useDeleteAssignmentMutation();

  const { title, video_title, totalMark, id } = asDetails || {};
  ...
  const handleDelete = (itemId) => {
    if (!isLoading) deleteAssignment(itemId);
  };
  ...
  return (
    <tr>
      <td className="table-td">{textExcerpt(title, 7)}</td>
      <td className="table-td">{textExcerpt(video_title, 7)}</td>
